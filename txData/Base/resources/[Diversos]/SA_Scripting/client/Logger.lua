            ...
            elseif not IsPedShooting(playerped) and fireCount ~= 0 and timeout ~= 0 then
                ...
                if fireCount ~= 0 and timeout == 0 then
                    if not armas[tostring(fireWeapon)] then
                        TriggerServerEvent('SA:Server:Logger:Tiros', 'Não encontrada', fireCount)
                        return
                    end
                    ...
                    if isLoggedWeapon then
                        TriggerServerEvent('SA:Server:Logger:Tiros', armas[tostring(fireWeapon)],
                            fireCount)
                    end
                    fireCount = 0
                end
    ...
    while true do
        ...
        if IsEntityDead(iPed) then
            if not hasRun then
                ...
                TriggerServerEvent('SA:Server:Logger:Morte', DeathReason, GetPlayerServerId(killer))
            end
        else
            Wait(200)
            hasRun = false
...
CreateThread(function()
    ...
    while true do
        ...
        if health > GetEntityHealth(PlayerPedId()) then
            ...
            print("------------------")
            TriggerServerEvent('SA:Server:Logger:Damage', math.floor((health - newHealth) / 2))
            health = newHealth
            Wait(500)
        else
            Wait(1000)
