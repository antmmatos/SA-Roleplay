...
function SendWebhookMessage(d, e)
    if d ~= nil and d ~= "" then
        PerformHttpRequest(d, function(f, g, h)
    ...
    for m, n in pairs(Config.gas_station_locations) do
        if not Config.gas_station_types[n.type] then
            if Config.lang == "br" then
                print(
                    "^8[" .. GetCurrentResourceName() .. "] Erro detectado no seu arquivo de configuracao, o tipo '" ..
                        n.type .. "' nao esta cadastrado em Config.gas_station_types^7")
            else
                print("^8[" .. GetCurrentResourceName() .. "] Error detected in your configuration file, the type '" ..
...
Citizen.CreateThread(function()
    ...
    while Config.clear_gas_stations.active do
        ...
        for m, n in pairs(p) do
            ...
            if n.stock < Config.gas_station_types[Config.gas_station_locations[n.gas_station_id].type].stock_capacity *
                Config.clear_gas_stations.min_stock_amount / 100 then
                if n.timer + Config.clear_gas_stations.cooldown * 60 * 60 < os.time() then
                    ...
                    MySQL.Sync.execute(o, {
                        ["@gas_station_id"] = n.gas_station_id
                    })
                    ...
                    SendWebhookMessage(Config.webhook,
                        Lang[Config.lang]["logs_lost_low_stock"]:format(n.gas_station_id, n.stock,
                            os.date("%d/%m/%Y %H:%M:%S", n.timer), n.user_id ..
                                os.date(
...
AddEventHandler("gas_station:getData", function(r)
    if c then
        ...
        if t then
            ...
            local query = MySQL.Sync.fetchAll(o, {
                ["@gas_station_id"] = r
            })
...
AddEventHandler("gas_station:buyMarket", function(r)
    if c then
        ...
        if money >= u then
            ...
            SendWebhookMessage(Config.webhook, Lang[Config.lang]["logs_bought"]:format(r, t ..
                os.date(
                    "\n[" .. Lang[Config.lang]["logs_date"] .. "]: %d/%m/%Y [" .. Lang[Config.lang]["logs_hour"] ..
                        "]: %H:%M:%S")))
        else
            TriggerClientEvent("gas_station:Notify", source, "negado",
                Lang[Config.lang]["insufficient_funds_store"]:format(u))
        end
    end
end)
RegisterServerEvent("gas_station:getJob")
AddEventHandler("gas_station:getJob", function(r)
    local source = source
    local s = ESX.GetPlayerFromId(source)
    local t = s.identifier
    if t then
...
AddEventHandler("gas_station:startJob", function(r, w)
    local source = source
...
AddEventHandler("gas_station:failed", function(x)
    if x then
...
AddEventHandler("gas_station:startContract", function(r, p)
    local source = source
    if b[source] == nil then
        b[source] = true
        ...
        if query[1].truck_upgrade >= y.ressuply[p.ressuply_id].truck_level then
            local z = y.ressuply[p.ressuply_id].liters +
                          math.floor(
            ...
            if p.type == 1 then
                ...
                if tryGetMarketMoney(r, B) then
                    insertBalanceHistory(r, 1, Lang[Config.lang]["buy_products_expenses"]:format(
                        y.ressuply[p.ressuply_id].name, z), B)
                    TriggerClientEvent("gas_station:startContract", source, z, query[1].truck_upgrade, p.ressuply_id,
                        p.type)
                else
                ...
                if tonumber(query[1].stock) >= z then
                    local o = "UPDATE `gas_station_business` SET stock = @stock WHERE gas_station_id = @gas_station_id"
                    MySQL.Sync.execute(o, {
                        ["@gas_station_id"] = r,
                        ["@stock"] = tonumber(query[1].stock) - z
                    })
                    TriggerClientEvent("gas_station:startContract", source, z, query[1].truck_upgrade, p.ressuply_id,
                        p.type)
                else
                    TriggerClientEvent("gas_station:Notify", source, "negado", Lang[Config.lang]["not_enought_stock"])
                end
            end
        else
...
AddEventHandler("gas_station:finishContract", function(r, z, C, D, type, x)
    local source = source
    local o =
    ...
    if type == 1 then
        if query[1].stock + z <= Config.gas_station_types[Config.gas_station_locations[r].type].stock_capacity +
            Config.gas_station_types[Config.gas_station_locations[r].type].upgrades.stock.level_reward[query[1]
                .stock_upgrade] then
            E = E + z
        else
            z = Config.gas_station_types[Config.gas_station_locations[r].type].stock_capacity +
                    Config.gas_station_types[Config.gas_station_locations[r].type].upgrades.stock.level_reward[query[1]
                        .stock_upgrade] - query[1].stock
            E = E + z
        ...
        local u = Config.gas_station_types[Config.gas_station_locations[r].type].ressuply[C].price_per_liter_to_export *
                      z
        giveMarketMoney(r, u)
        local o =
        ...
        insertBalanceHistory(r, 0,
            Lang[Config.lang]["exported_income"]:format(
                Config.gas_station_types[Config.gas_station_locations[r].type].ressuply[C].name, z), u)
    end
end)
...
AddEventHandler("gas_station:createJob", function(r, p)
    ...
    if b[source] == nil then
        ...
        local F = MySQL.Sync.fetchAll(o, {
            ["@gas_station_id"] = r
        })[1].qtd
        if F < 50 then
            local o = "SELECT relationship_upgrade FROM `gas_station_business` WHERE gas_station_id = @gas_station_id"
            local query = MySQL.Sync.fetchAll(o, {
                ["@gas_station_id"] = r
            })
            local u = Config.gas_station_types[Config.gas_station_locations[r].type].ressuply_deliveryman
                          .price_per_liter * p.amount
            local A =
                Config.gas_station_types[Config.gas_station_locations[r].type].upgrades.relationship.level_reward[query[1]
...
AddEventHandler("gas_station:deleteJob", function(r, p)
    local source = source
    if b[source] == nil then
        b[source] = true
        local o = "SELECT name,reward,amount,progress FROM `gas_station_jobs` WHERE id = @id;"
        query = MySQL.Sync.fetchAll(o, {
            ["@id"] = p.job_id
        })
        if query[1] then
            if not query[1].progress then
                local o = "DELETE FROM `gas_station_jobs` WHERE id = @id;"
                MySQL.Sync.execute(o, {
                    ["@id"] = p.job_id
                })
                ...
                local u = Config.gas_station_types[Config.gas_station_locations[r].type].ressuply_deliveryman
                              .price_per_liter * query[1].amount
                local A = Config.gas_station_types[Config.gas_station_locations[r].type].upgrades.relationship
                              .level_reward[v[1].relationship_upgrade]
                A = math.floor(u * A / 100)
...
AddEventHandler("gas_station:applyPrice", function(r, p)
    local source = source
    if b[source] == nil then
        b[source] = true
        local s = ESX.GetPlayerFromId(source)
...
AddEventHandler("gas_station:buyUpgrade", function(r, p)
    local source = source
    if b[source] == nil then
        b[source] = true
        local s = ESX.GetPlayerFromId(source)
        local t = s.identifier
        if t then
            local o = "SELECT " .. p.id .. "_upgrade FROM `gas_station_business` WHERE gas_station_id = @gas_station_id"
            local query = MySQL.Sync.fetchAll(o, {
                ["@gas_station_id"] = r
            })[1]
            if query[p.id .. "_upgrade"] < 5 then
                ...
                if tryGetMarketMoney(r, z) then
                    local o = "UPDATE `gas_station_business` SET " .. p.id .. "_upgrade = " .. p.id ..
                                  "_upgrade + 1 WHERE gas_station_id = @gas_station_id"
                    MySQL.Sync.execute(o, {
                        ["@gas_station_id"] = r
                    })
                    insertBalanceHistory(r, 1, Lang[Config.lang]["upgrade_expenses"]:format(
                        Lang[Config.lang][p.id .. "_upgrade"]), z)
                    openUI(source, r, true)
                else
                    TriggerClientEvent("gas_station:Notify", source, "negado", Lang[Config.lang]["insufficient_funds"])
                end
...
AddEventHandler("gas_station:withdrawMoney", function(r)
    local source = source
    if b[source] == nil then
        b[source] = true
        local s = ESX.GetPlayerFromId(source)
        ...
        if t then
            ...
            if z and z > 0 then
                ...
                SendWebhookMessage(Config.webhook, Lang[Config.lang]["logs_money_withdrawn"]:format(r, z, t ..
                    os.date(
                        "\n[" .. Lang[Config.lang]["logs_date"] .. "]: %d/%m/%Y [" .. Lang[Config.lang]["logs_hour"] ..
                            "]: %H:%M:%S")))
                openUI(source, r, true)
    ...
    if b[source] == nil then
        ...
        if t then
            ...
            if query.user_id == t then
                ...
                SendWebhookMessage(Config.webhook, Lang[Config.lang]["logs_close"]:format(r, t ..
                    os.date(
                        "\n[" .. Lang[Config.lang]["logs_date"] .. "]: %d/%m/%Y [" .. Lang[Config.lang]["logs_hour"] ..
                            "]: %H:%M:%S")))
            else
...
function tryGetMarketMoney(user_id, amount)
    ...
    if tonumber(query.money) >= amount then
        ...
        MySQL.Sync.execute(o, {
            ["@money"] = tonumber(query.money) - amount,
            ["@amount"] = amount,
            ["@gas_station_id"] = user_id
        })
        return true
...
function openUI(source, r, J)
    ...
    if t then
        ...
        query.gas_station_business = MySQL.Sync.fetchAll(o, {
            ["@gas_station_id"] = r
        })[1]
        local o = "SELECT * FROM `gas_station_jobs` WHERE gas_station_id = @gas_station_id"
        query.gas_station_jobs = MySQL.Sync.fetchAll(o, {
            ["@gas_station_id"] = query.gas_station_business.gas_station_id
        })
        local o = "SELECT * FROM `gas_station_balance` WHERE gas_station_id = @gas_station_id ORDER BY id DESC"
        query.gas_station_balance = MySQL.Sync.fetchAll(o, {
            ["@gas_station_id"] = query.gas_station_business.gas_station_id
        })
        query.config = {}
        ...
        if query.gas_station_business.stock <
...
AddEventHandler("gas_station:vehicleLock", function()
    local source = source
