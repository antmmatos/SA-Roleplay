...
AddEventHandler("vrp_legacyfuel2:pagamento", function(price, galao, vehicle, fuel, fuel2, key)
	...
	if key then
		...
		if query and query[1] then
			...
			local sql =
			"UPDATE `gas_station_business` SET total_visits = total_visits + 1 WHERE gas_station_id = @gas_station_id";
		...
		if galao or output.stock > amount then
			...
			if money >= price then
				...
				if galao then
					...
					TriggerClientEvent("gas_station:Notify", source, "sucesso",
						"Pagou <b>$" .. price .. " </b> pelo <b>Galão</b>.", 8000)
				else
					if key and query and query[1] then
						local sql =
						"UPDATE `gas_station_business` SET stock = @stock, customers = customers + 1, money = money + @price, total_money_earned = total_money_earned + @price, gas_sold = gas_sold + @amount WHERE gas_station_id = @gas_station_id";
						MySQL.Sync.execute(sql,
							{ ['@gas_station_id'] = key, ['@stock'] = ((output.stock) - amount), ['@price'] = price,
								['@amount'] = amount });

						local sql =
						"INSERT INTO `gas_station_balance` (gas_station_id,income,title,amount,date) VALUES (@gas_station_id,@income,@title,@amount,@date)";
						MySQL.Sync.execute(sql,
							{ ['@gas_station_id'] = key, ['@income'] = 0, ['@title'] = "Fuel sold (" .. amount .. " Liters)",
								['@amount'] = price, ['@date'] = os.time() });
					end

					TriggerClientEvent('syncfuel', -1, vehicle, fuel)
					TriggerClientEvent("gas_station:Notify", source, "sucesso",
						"Pagou <b>$" .. price .. " </b> in " .. amount .. " litros.", 8000)
				end
			else
			...
			TriggerClientEvent("gas_station:Notify", source, "negado",
				"Stock de combustivel insuficiente (Max: " .. output.stock .. " litros).", 8000)
		end
	end
end)
